...
const props = defineProps(['row']) as { row: ArticleItem };
...
const formEdit = ref({
  type_id: props.row.type_id,
  is_dfat: props.row.is_dfat,
})
...
const handleDelte = () => {
  Modal.confirm({
    title: '删除',
    content: `确定删除文章  ${props.row.article_title}  吗？`,
    onOk() {
      deleteArticle(props.row.article_id)
    }
  })
}
...
const handleEditText = () => {
  router.push({
    path: `/layout/article-update/${props.row.article_id}`,
  })
}

const handleEditOk = async () => {
  const { type_id, is_dfat } = formEdit.value;
  const { code } = await service.updateArticle(props.row.article_id, { type_id, is_dfat });
  if (code === 200) {
